...
class AppStore extends Component {
  ...
  render() {
    ...
    const filterApps = this.getFilterApps()
    const getAppsFilterd = filterApps.filter(eachAppList =>
      eachAppList.appName.toLowerCase().includes(searchInput),
    )
    ...
    return (
      <div className="main-container">
        ...
        <ul className="tablist-style">
          {tabsList.map(eachList => (
            <TabItem
              key={eachList.tabId}
              eachList={eachList}
              setCurrentActiveTab={this.setCurrentActiveTab}
              activeCheck={eachList.tabId === activeTabId}
            />
          ))}
        </ul>

        <ul className="app-List">
          {getAppsFilterd.map(eachItem => (
